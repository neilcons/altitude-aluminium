...
class Bifold_Door extends Database_Table_Object
{
	protected $_table_name = 'bifold_door';
	...
	public function get_hidden_fields($which = "all")
	{
		...
		switch ($which) {
			case 'step2':
				// return elements(array('opening', 'bifold_scheme_id', 'sliding_direction', 'width', 'height', 'master_handle', 'slave_handle_colour_id', 'tint_id', 'energy_efficient', 'triple_glazing', 'glass_id', 'anti_sun', "blind_id", 'midrail', 'internal_colour_id', 'external_colour_id', 'midrail', 'trickle_vent', 'hinge_colour_id', 'cill', 'magnets', 'threshold_id'), $this->_data, '');
				return elements(array('opening', 'bifold_scheme_id', 'width', 'height', 'master_handle', 'slave_handle_colour_id', 'tint_id', 'energy_efficient', 'triple_glazing', 'glass_id', 'anti_sun', "blind_id", 'midrail', 'internal_colour_id', 'external_colour_id', 'midrail', 'trickle_vent', 'hinge_colour_id', 'cill', 'magnets', 'threshold_id'), $this->_data, '');
			default:
				return $this->_data;
				break;
		}
	...
	public function has_handle()
	{
		...
		if( $this->bifold_scheme->number_panels_left%2 == 0 && $this->bifold_scheme->number_panels_right%2 == 0 ) {
			return false;
		} else {
			return true;
		}
	...
	public function _get_bifold_scheme()
	{
		$bifold_scheme = new Bifold_Scheme();
		$bifold_scheme->load($this->bifold_scheme_id);

		return $bifold_scheme;
	}
	...
	public function get_panel_with_handle()
	{
		...
		if( $this->sliding_direction == 'left' ) {
			...
			if( $this->handle_position == 'left' ) {
				// handle on left
				$panel = $this->bifold_scheme->number_panels_left;
			} else {
				// handle on right
				$panel = $this->bifold_scheme->number_panels - $this->bifold_scheme->number_panels_right;
			}
		...
		// set position of handle for RIGHT sliding selection
			...
			if( $this->handle_position == 'left' ) {
				// handle on left
				$panel = $this->bifold_scheme->number_panels - $this->bifold_scheme->number_panels_left;
			} else {
				// handle on right
				$panel = $this->bifold_scheme->number_panels_right;
			}
		}
...
*/
	...
	{
		return $this->bifold_scheme->number_panels;
	}
	...
	{
		return $this->width - $this->bifold_scheme->width_deduction * $this->bifold_scheme->number_panels;
	}

	public function get_glazing_height()
	{
		$height =  $this->height - $this->bifold_scheme->height_deduction;
	...
	public function get_min_max_widths_and_heights() 
	{
		...
		$oQuery = $CI->db->query('SELECT MIN(min_width) AS min_width, MAX(max_width) AS max_width FROM bifold_scheme');
		...
		$oQuery = $CI->db->query('SELECT MIN(height_up_to) AS min_height, MAX(height_up_to) AS max_height FROM bifold_scheme_price_matrix');
	...
	public function get_latest_orders()
	{
		...
		*/
		...
		// build the query
		$DB->select('bifold_door.id, bifold_door.date, bifold_door.your_name, bifold_door.your_address, bifold_door.your_postcode, bifold_door.your_email, bifold_door.your_tel');
		$DB->from('bifold_door');
		$DB->where('bifold_door.id >', 1);
		$DB->order_by('bifold_door.id', 'desc');
	...
	public function get_bifold($id)
	{
		...
		,	'bifold_threshold.name'					=> 'threshold'
		,	'bifold_door.supply_type'				=> 'supply_type'
		,	'bifold_door.your_name'					=> 'your_name'
		,	'bifold_door.your_address'				=> 'your_address'
		,	'bifold_door.your_postcode'				=> 'your_postcode'
		,	'bifold_door.your_email'				=> 'your_email'
		,	'bifold_door.your_tel'					=> 'your_telephone'
		,	'bifold_door.quote_price'				=> 'quote_price'
		,	'bifold_door.quote_price_with_vat'		=> 'quote_price_with_vat'
		);
		...
		$DB->select($sSelectFields);
		$DB->from('bifold_door');
		$DB->join('bifold_scheme', 'bifold_scheme.id = bifold_door.bifold_scheme_id', 'left');
		$DB->join('colour AS external_colour', 'external_colour.id = bifold_door.external_colour_id', 'left');
		$DB->join('colour AS internal_colour', 'internal_colour.id = bifold_door.internal_colour_id', 'left');
		$DB->join('bifold_hardware_colour AS traffic_handle_colour', 'traffic_handle_colour.id = bifold_door.master_handle', 'left');
		$DB->join('bifold_hardware_colour AS hinge_colour', 'hinge_colour.id = bifold_door.hinge_colour_id', 'left');
		$DB->join('bifold_hardware_colour AS slave_colour', 'slave_colour.id = bifold_door.slave_handle_colour_id', 'left');
		$DB->join('tint', 'tint.id = bifold_door.tint_id', 'left');
		$DB->join('bifold_cill', 'bifold_cill.id = bifold_door.cill', 'left');
		$DB->join('bifold_threshold', 'bifold_threshold.id = bifold_door.threshold_id', 'left');
		$DB->where('bifold_door.id', $id);
